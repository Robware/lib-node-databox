...
exports.NewKeyValueClient = function (reqEndpoint, enableLogging) {
    ...
    let kvc = {
        ...
        RegisterDatasource: function (DataSourceMetadata) {
            return exports.LiveMode ? _registerDatasource(this.zestClient,this.zestEndpoint,'kv',DataSourceMetadata):new Promise((s,j)=>{s()});
        },
        GetDatasourceCatalogue: function () {
            return exports.LiveMode ? _getDatasourceCatalogue(this.zestClient,this.zestEndpoint):new Promise((s,j)=>{s()});
        }
    };
...
let HypercatToSourceDataMetadata = function (hyperCatString) {
    ...
    });//:new Promise((s,j)=>{s({DataSourceURL:"", DataSourceMetadata:{ DataSourceID:""}})});
...
function _makeTestPath(path){
    return `/tmp/databox_store${path}`;
}

function _writeTest(path, payload, resolve, reject){
    console.log("Got data to save", path, payload);
    fs.writeFile(_makeTestPath(path), payload, (err) => {
        if(err) {
            console.log(err);
            reject(err);
        }
...
function _writeLive(zestClient,zestEndpoint, path, tokenPath, payload, resolve, reject){
...
let _write = function (zestClient,zestEndpoint, path, tokenPath, payload, contentFormat) {
    return new Promise((resolve, reject) => {
        ...
        exports.LiveMode ?
            _writeLive(zestClient,zestEndpoint, path, tokenPath, payload, resolve, reject):
            _writeTest(path, payload, resolve, reject);
    });
};

function _readParse(data, contentFormat){
    return data !== null && contentFormat == 'JSON'?
        JSON.parse(resp):
        data;
}

function _readTest(path, contentFormat, resolve, reject){
    console.log("Reading from data store:", path);
    fs.readFile(_makeTestPath(path), (err, data)=>{
        if (err){
            console.log("Failed read:", err);
            reject(err);
            return;
        }
        resolve(_readParse(data.toString("utf8"), contentFormat));
        console.log("Successful read");
    });
}

function _readLive(zestClient,zestEndpoint, path, tokenPath, contentFormat, resolve, reject){
    let endPoint = url.parse(zestEndpoint);
    utils.requestToken(endPoint.hostname,tokenPath,"GET")
    .then((token)=>{
        zestClient.Get(token,path,contentFormat)
        .then((resp)=>{
            resolve(_readParse(resp));
        })
        .catch((err)=>{
            utils.invalidateToken(endPoint.hostname,path,"GET");
...
let _read = function (zestClient,zestEndpoint, path, tokenPath, contentFormat) {
    return new Promise((resolve, reject) => {
        ...
        exports.LiveMode ?
            _readLive(zestClient,zestEndpoint, path, tokenPath, contentFormat, resolve, reject):
            _readTest(path, contentFormat, resolve, reject);
    });
};
...
function _observeTest(path, resolve, reject){
    console.log("Got path to observe:", path);
    let emitter = new EventEmitter();

    path=path.replace(/\*$/, "");
    var base=_makeTestPath(path.replace(/\/([a-z]*?)$/,"/"));

    fs.watch(_makeTestPath(path), (event, filename)=>{
...
function _observeLive(zestClient, zestEndpoint, path, timeOut, contentFormat, resolve, reject){
    if(checkContentFormat(contentFormat) === false) {
...
let _observe = function (zestClient, zestEndpoint, path, timeOut = 60, contentFormat = "JSON") {
    return new Promise((resolve, reject) => {
        exports.LiveMode ?
            _observeLive(zestClient, zestEndpoint, path, timeOut, contentFormat, resolve, reject):
            _observeTest(path, resolve, reject);
    });
}
